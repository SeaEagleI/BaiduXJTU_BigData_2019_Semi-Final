...
std_log = './log/Net4_TTA_fold1.log'
...
def train(Dataloader,model, criterion, optimizer, epoch):
    ...
    for i, (input_O, input_H, input_V, input_vis, anos) in enumerate(Dataloader):
        data_time.update(time.time() - end)
        
        input_O = input_O.cuda()
        ...
        with torch.no_grad():
            input_O_var = Variable(input_O.cuda())
            input_H_var = Variable(input_H.cuda())
            input_V_var = Variable(input_V.cuda())
            input_vis_var = Variable(input_vis.cuda())
            target_var = Variable(target.cuda())

        input_tensor = [input_O_var, input_H_var, input_V_var]

        for input_img_var in input_tensor:
            # compute output
            ...
            losses.update(loss.item(), input_O.size(0))
            top1.update(prec1.item(), input_O.size(0))
            top5.update(prec5.item(), input_O.size(0))
#    for i, (input_O, input_H, input_V, input_vis, anos) in enumerate(Dataloader):
#        data_time.update(time.time() - end)
#        target = anos.cuda(async=True)
#
...
#        
#        # compute output
#        # output = model(input_O_var.cuda(), input_H_var.cuda(), input_V_var.cuda(), input_vis_var.cuda())
#        output = model(torch.cat([input_O_var, input_H_var, input_V_var],dim = 0).cuda(), input_vis_var.cuda())
#        #print(target_var)
#        loss = criterion(output, target_var)
#
#        # measure accuracy and record loss
...
#    for i, (input_O, input_H, input_V, input_vis, target) in enumerate(val_loader):
#        input_O = input_O.cuda()
#        input_H = input_H.cuda()
#        input_V = input_V.cuda()
#        input_vis = input_vis.cuda()
...
def main():
    ...
    # Dataset
    Aug = Augmentation()
    Dataset_train = MM_BDXJTU2019_TTA(root = args.dataset_root, mode = 'train', 
                                  transform = None, TRAIN_IMAGE_DIR = 'train_image_raw')
    ...
    criterion = nn.CrossEntropyLoss(weight = class_ration).cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        COMMON_MODEL_PATH = op.join(MODEL_DIR,'SGD_TTA_fold1_{}.pth'.format(epoch+1))
        BEST_MODEL_PATH   = op.join(BEST_DIR,'{}_SGD_TTA_fold1_{}.pth'.format(MODEL_NAME,epoch+1))
