...
def LoadJson(json_path):
    raw_json = open(json_path).read()[1:-1]+','
    List = [(line+']]').split(':') for line in tqdm(raw_json.split(']],')[:-1])]
    return List
...
STr_User_Txts = LoadJson(STr_User_Txts_path) if op.isfile(STr_User_Txts_path) else {}
print('Loaded {} Users from {}.'.format(len(STr_User_Txts),STr_User_Txts_path))
PTr_User_Txts = LoadJson(PTr_User_Txts_path) if op.isfile(PTr_User_Txts_path) else {}
...
User_Txts = {}
#Merge STr_User_Txts to User_Txts
_len = len(STr_User_Txts)
for i in tqdm(range(_len)):
    user,txtlist = STr_User_Txts[-1]
#    print(user,eval(txtlist))
    User_Txts[user] = LabelTxtlist(eval(txtlist),'S')
    del STr_User_Txts[-1]
#Merge PTr_User_Txts to User_Txts
_len = len(PTr_User_Txts)
for i in tqdm(range(_len)):
    user,txtlist = PTr_User_Txts[-1]
    MergeDict(User_Txts,user,eval(txtlist))
    del PTr_User_Txts[-1]
